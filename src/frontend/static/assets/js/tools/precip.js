...
const locationIcon = new L.icon({
	iconUrl: 'assets/images/tools/location_pin.svg',
	shadowUrl: 'assets/images/tools/location_pin_shadow.svg',
...
async function init() {
	...
	if (is_touch_enabled())
		...
		map.addEventListener('click', (event) => setLocation([event.latlng?.lat, event?.latlng?.lng]));
	...
	const clearLocationBtn = document.querySelector(
		'#map__contents__navigate__search .icon[alt="Clear"]'
	);
...
function generateFileContent() {
	...
	const values = periods
		.map((period) => {
			let res = [
				`${period}-Year in today: ${locationData?.period[years[0]]?.years[period].present}`
			];
			res = res.concat(
...
function download() {
	...
	element.setAttribute(
		'href',
		'data:text/plain;charset=utf-8,' + encodeURIComponent(generateFileContent())
	);
...
async function handleFind(event) {
	...
	for (i = 0; i < arrayLength; i++) {
		...
		b.addEventListener('click', async function (e) {
			...
			const latLng = [
				this.querySelector('input[name=lat]')?.value,
				this.querySelector('input[name=lng]')?.value
			];
			closeAllLists();
...
function handleDetailsPosition(event) {
	...
	mapEl.hasAttribute('data-details-as-sheet')
		? delete mapEl.dataset.detailsAsSheet
...
async function buildPage(stateObj, addToHistory) {
	...
	if (addToHistory)
		history.pushState(stateObj, '', page.slug + (language ? '?lang=' + language : ''));
...
function drawPrecipitactionGraphDOM(data, defaultPeriod = 2030, uncert = false) {
	...
	for (let years in yearObj) {
		...
		if (uncert) {
			...
			overlap.innerHTML =
				'<div style="width: 3%; height: 100%; margin: 0 auto; border-right: #17527c 1px solid;">&nbsp;</div>';
